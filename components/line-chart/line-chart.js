// lunadom/components/line-chart/line-chart.js
 ...
 * @cssprop --luna-line-bg              - Component background (default: transparent)
 * @cssprop --luna-line-grid-color      - Horizontal grid line colour (default: rgba(255,255,255,.05))
 * @cssprop --luna-line-axis-color      - Axis line colour (default: rgba(255,255,255,.1))
 * @cssprop --luna-line-text-color      - Tick and axis label colour (default: #666)
 * @cssprop --luna-line-color           - Primary series stroke colour (default: #2563eb)
 * @cssprop --luna-line-area-opacity    - Area fill opacity (default: 0.12)
 * @cssprop --luna-line-stroke-width    - Line stroke width in px (default: 2)
 * @cssprop --luna-line-dot-r           - Dot radius in SVG units (default: 3)
 * @cssprop --luna-line-dot-border      - Dot border/stroke colour (default: var(--luna-line-bg, #0a0a0a))
 * @cssprop --luna-line-series-colors   - Comma-separated colour list for multi-series lines.
 *                                        Overrides the built-in palette.
 * @cssprop --luna-line-goal-color      - Reference line colour (default: #f59e0b)
 * @cssprop --luna-line-goal-dash       - SVG stroke-dasharray for the goal line (default: 5,4)
 * @cssprop --luna-line-tooltip-bg      - Tooltip background (default: #1e1e1e)
 * @cssprop --luna-line-tooltip-fg      - Tooltip text colour (default: #eee)
 * @cssprop --luna-line-tooltip-border  - Tooltip border colour (default: #333)
 * @cssprop --luna-line-legend-gap      - Gap between legend items (default: 1rem)
 * @cssprop --luna-line-cursor-color    - Vertical crosshair line colour (default: rgba(255,255,255,.1))
 *
 * Events:
 * @event luna-line-hover - Fired on data-point hover. detail: { label, value, series, index } or null on leave.
 * @event luna-line-click - Fired on data-point click. detail: { label, value, series, index }.
 */
class LunaLineChart extends HTMLElement {
  ...
  _palette() {
    const custom = this._cssVar('--luna-line-series-colors', '');
    if (custom) {
      return custom.split(',').map(s => s.trim()).filter(Boolean);
    }
    return LunaLineChart._defaultPalette;
  }
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          ...
          --luna-line-bg:             transparent;
          --luna-line-grid-color:     rgba(255, 255, 255, 0.05);
          --luna-line-axis-color:     rgba(255, 255, 255, 0.1);
          --luna-line-text-color:     #666;
          --luna-line-color:          #2563eb;
          --luna-line-area-opacity:   0.12;
          --luna-line-stroke-width:   2;
          --luna-line-dot-r:          3;
          --luna-line-dot-border:     #0a0a0a;
          --luna-line-goal-color:     #f59e0b;
          --luna-line-goal-dash:      5, 4;
          --luna-line-tooltip-bg:     #1e1e1e;
          --luna-line-tooltip-fg:     #eee;
          --luna-line-tooltip-border: #333;
          --luna-line-legend-gap:     1rem;
          --luna-line-cursor-color:   rgba(255, 255, 255, 0.1);
        }
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .line {
          ...
          stroke-width: var(--luna-line-stroke-width);
        }

        .area {
          stroke: none;
          opacity: var(--luna-line-area-opacity);
        }
        ...
        .cursor-line {
          stroke: var(--luna-line-cursor-color);
          stroke-width: 1;
          pointer-events: none;
          opacity: 0;
        ...
        .goal-label-text {
          font-size: 0.68em;
          fill: var(--luna-line-goal-color);
        }

        .legend {
          display: flex;
          flex-wrap: wrap;
          gap: 0.25rem var(--luna-line-legend-gap);
          font-size: 0.78em;
          color: var(--luna-line-text-color);
          padding: 0 4px;
        }
        ...
        .tooltip {
          position: fixed;
          background: var(--luna-line-tooltip-bg);
          color: var(--luna-line-tooltip-fg);
          font-family: inherit;
          font-size: 0.6875rem;
          padding: 6px 11px;
          border-radius: 6px;
          border: 1px solid var(--luna-line-tooltip-border);
          pointer-events: none;
          white-space: nowrap;
          opacity: 0;
  ...
  _draw() {
    ...
    const gridColor = this._cssVar('--luna-line-grid-color', 'rgba(255,255,255,.05)');
    const axisColor = this._cssVar('--luna-line-axis-color', 'rgba(255,255,255,.1)');
    ...
    if (isMulti) {
      series.forEach((s, si) => {
        ...
        if (showDots) {
          pts.forEach((p, i) => {
            const dotBorder = this._cssVar('--luna-line-dot-border', '#0a0a0a');
            out += `<circle cx="${p[0]}" cy="${p[1]}" r="${this._cssVar('--luna-line-dot-r', '3')}" class="dot" fill="${color}" stroke="${dotBorder}" data-series="${si}" data-idx="${i}" />`;
          });
        }
      ...
      const color = this._cssVar('--luna-line-color', '#2563eb');
      ...
      if (showDots) {
        pts.forEach((p, i) => {
          const dotBorder = this._cssVar('--luna-line-dot-border', '#0a0a0a');
          out += `<circle cx="${p[0]}" cy="${p[1]}" r="${this._cssVar('--luna-line-dot-r', '3')}" class="dot" fill="${color}" stroke="${dotBorder}" data-series="0" data-idx="${i}" />`;
        });
      }
    ...
    if (!isNaN(goalVal)) {
      const gc  = this._cssVar('--luna-line-goal-color', '#f59e0b');
      const gd  = this._cssVar('--luna-line-goal-dash', '5,4');
  ...
  _bindEvents(svg, items, series, isMulti, precision, palette) {
    ...
    const defaultColor = this._cssVar('--luna-line-color', '#2563eb');
    ...
    svg.querySelectorAll('.hit').forEach(el => {
      el.addEventListener('mouseenter', e => {
        ...
        this.dispatchEvent(new CustomEvent('luna-line-hover', {
          bubbles:  true,
          composed: true,
      ...
      el.addEventListener('mouseleave', () => {
        ...
        const dot = svg.querySelector(`.dot[data-series="${si}"][data-idx="${idx}"]`);
        if (dot) { dot.setAttribute('r', this._cssVar('--luna-line-dot-r', '3')); }
      ...
      el.addEventListener('click', () => {
        ...
        this.dispatchEvent(new CustomEvent('luna-line-click', {
          bubbles:  true,
          composed: true,
          detail: { label: item.label, value, series: isMulti ? series[si] : 'value', index: idx },
        }));
...
customElements.define('luna-line-chart', LunaLineChart);
